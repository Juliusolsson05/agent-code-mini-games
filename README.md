...
A five-game arcade inside [Agent Code](https://github.com/Juliusolsson05/agent-code). Everything runs locally in the extension sandbox: procedural artwork, synthesized audio, bundled word lists, and saved progress, with no network requests or external assets during play.
...
- **Blockfall:** a falling-block puzzle that plays by modern guideline rules, with its own look and synthesized sound.
  - **Modes:** Marathon (150 lines, one level every 10), Sprint (40 lines against the clock) and Ultra (highest score in 3 minutes). Each mode keeps its own record.
  - **Rules:** Super Rotation System with wall kicks, 7-bag randomizer, hold, five previews, ghost piece, and extended lock down (500 ms, 15 resets). T-spins and minis, back-to-back, combos and perfect clears score as the guideline does.
  - **Controls:** ← → move; ↓ soft drop; Space hard drop; ↑ / X rotate clockwise; Z rotate counter-clockwise; C / Shift hold; Esc / P pause; R restart; M toggles sound. DAS, ARR, soft drop speed and the ghost piece are adjustable, and settings persist.

The launcher and game controls follow the host theme. The garden, felt, cards, chips, classic minefield, typing page preview and Blockfall's night-blue well keep their artwork palettes. Keyboard input stays within the focused game.

## Install

In Agent Code → **Settings → Apps**, install from GitHub using `Juliusolsson05/agent-code-mini-games`, or choose **Load folder…** for a local checkout. Open **Play Mini Games**, **Play Snake**, **Play Blackjack**, **Play Minesweeper**, **Play Typing Test**, or **Play Blockfall** from the command palette.
...
Open the printed local URL. `/dev/?game=snake`, `blackjack`, `minesweeper`, `typing`, or `blockfall` opens a specific game. `/dev/gallery.html` shows all screens with theme switches. `/dev/?build=production` exercises the built extension’s activation, commands, and view mount using the browser storage adapter.
...
Set `CHROME_PATH` to an existing Chrome executable to use it for browser checks. Those checks exercise real controls, pointer order, touch, pause, replay, records, split hands, typing and correction, Blockfall's pause and top out, and the production bundle; they do not replace verification inside the Electron extension host. The browser harness uses its own localStorage namespace and never reads Agent Code’s saved games.
